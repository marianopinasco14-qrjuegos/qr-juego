...
const GAMES=[{id:"RULETA",label:"Ruleta",emoji:"🎡",desc:"Ruleta giratoria"},{id:"RASCA_Y_GANA",label:"Rasca y Gana",emoji:"🎫",desc:"Tarjeta táctil"},{id:"SLOTS",label:"Tragamonedas",emoji:"🎰",desc:"Tres rodillos"}];
export default function NewCampaignPage() {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({ name:"", gameType:"RULETA", attemptsPerSession:3, primaryColor:"#7C3AED", secondaryColor:"#A78BFA", backgroundColor:"#1a1a2e", language:"es", ageGate:false, prizes:[{title:"",stock:10,priority:5,frequency:50,validDays:30}], consolePrize:{title:"",couponCode:""}, startDate:"", endDate:"", emailWinner:{subject:"🎉 ¡Ganaste un premio!",bodyHtml:"<h1>Felicitaciones {{name}}! Ganaste: {{prize}}</h1><p>Codigo: <strong>{{redemptionCode}}</strong></p><p>Valido hasta: {{expiresAt}}</p>"}, emailConsole:{subject:"Tu regalo te espera 🎁",bodyHtml:"<h1>Gracias por participar!</h1><p>Codigo: <strong>{{couponCode}}</strong></p>"}, upsellEnabled:false, upsellTitle:"", upsellPrice:0, upsellCurrency:"ARS", upsellLink:"", closedBehavior:"LEAD_MAGNET" });
  const upd=(d:any)=>setForm(p=>({...p,...d}));
  const canGo=()=>{if(step===1)return form.name.trim().length>=3;if(step===3)return form.prizes.some(p=>p.title.trim())&&form.consolePrize.title.trim().length>0;return true;};
      ...
      <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 min-h-64">
        ...
        {step===3&&<div className="space-y-4">
          ...
          {form.prizes.map((p,i)=><div key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">{[{k:"stock",l:"Stock"},{k:"frequency",l:"Cada X scans"},{k:"validDays",l:"Días válido"}].map(f=><div key={f.k}><label className="text-white/50 text-xs block mb-1">{f.l}</label><input type="number" min="1" value={(p as any)[f.k]} onChange={e=>{const ps=[...form.prizes];(ps[i] as any)[f.k]=parseInt(e.target.value)||1;upd({prizes:ps});}} className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-2 text-white text-sm focus:outline-none"/></div>)}</div>
          </div>)}
          <div className="border border-amber-500/30 bg-amber-500/10 rounded-xl p-4 space-y-2">
            <label className="text-white font-medium block">⭐ Premio consuelo (obligatorio)</label>
        ...
        {step===4&&<div className="space-y-4">
          <p className="text-white/60 text-sm">Variables: <code className="text-violet-300">{"{{name}}"}</code> <code className="text-violet-300">{"{{prize}}"}</code> <code className="text-violet-300">{"{{redemptionCode}}"}</code> <code className="text-violet-300">{"{{expiresAt}}"}</code> <code className="text-violet-300">{"{{couponCode}}"}</code></p>
          <div className="space-y-2"><p className="text-white font-medium">📧 Email Ganador</p><input value={form.emailWinner.subject} onChange={e=>upd({emailWinner:{...form.emailWinner,subject:e.target.value}})} className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" placeholder="Asunto"/><textarea rows={3} value={form.emailWinner.bodyHtml} onChange={e=>upd({emailWinner:{...form.emailWinner,bodyHtml:e.target.value}})} className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white text-sm font-mono focus:outline-none resize-none"/></div>
          <div className="space-y-2"><p className="text-white font-medium">📧 Email Consuelo</p><input value={form.emailConsole.subject} onChange={e=>upd({emailConsole:{...form.emailConsole,subject:e.target.value}})} className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white text-sm focus:outline-none" placeholder="Asunto"/><textarea rows={3} value={form.emailConsole.bodyHtml} onChange={e=>upd({emailConsole:{...form.emailConsole,bodyHtml:e.target.value}})} className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white text-sm font-mono focus:outline-none resize-none"/></div>
        </div>}
