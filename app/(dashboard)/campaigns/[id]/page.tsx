    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 min-h-64">
        ...
        {step===2&&<div className="space-y-5">
          ...
          <div><label className="text-white font-medium block mb-2">Logo de tu negocio (opcional)</label><input type="file" accept="image/*" onChange={async(e)=>{const f=e.target.files?.[0];if(!f)return;if(f.size>2000000){alert("La imagen no puede superar 2MB");return;}const fd=new FormData();fd.append("file",f);const r=await fetch("/api/upload",{method:"POST",body:fd});const d=await r.json();if(d.url)upd({logoUrl:d.url});}} className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none cursor-pointer"/>{form.logoUrl&&<img src={form.logoUrl} alt="logo" className="w-24 h-24 object-contain rounded-xl border border-white/10 mt-2"/>}</div>
        ...
        {step===3&&<div className="space-y-4">
          ...
          {form.prizes.map((p: any,i: number)=><div key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">{[{k:"stock",l:"Stock"},{k:"frequency",l:"Cada X scans"},{k:"validDays",l:"Días válido"}].map(f=><div key={f.k}><label className="text-white/50 text-xs block mb-1">{f.l}</label><input type="number" min="1" value={p[f.k]} onChange={e=>{const ps=[...form.prizes];ps[i][f.k]=parseInt(e.target.value)||1;upd({prizes:ps});}} className="w-full bg-white/10 border border-white/20 rounded-lg px-2 py-2 text-white text-sm focus:outline-none"/></div>)}</div>
