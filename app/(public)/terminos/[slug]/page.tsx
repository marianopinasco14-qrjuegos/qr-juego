...
export default async function TerminosPage({ params }: { params: { slug: string } }) {
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-white/40 text-sm mt-2">Los términos y condiciones no están disponibles.</p>
        </div>
      </div>
    );
  }
  ...
  const isSorteo = campaign.gameType === "SORTEO";

  const formatDate = (d: Date | null) => {
    if (!d) return "—";
    return d.toLocaleDateString("es-AR", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" });
  };
    ...
    <div className="min-h-screen" style={{ backgroundColor: bg }}>
      <div className="max-w-2xl mx-auto px-5 py-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-white/50 text-sm">Términos y condiciones</p>
        </div>
        ...
        {isSorteo ? (
          ...
          <div className="rounded-2xl p-6 space-y-4" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <h2 className="font-bold text-lg" style={{ color: primary }}>Datos del juego</h2>
            <div className="space-y-3">
              ...
              {campaign.endDate && (
                <div className="flex justify-between items-start gap-4">
                  <span className="text-white/50 text-sm shrink-0">Válido hasta</span>
                  <span className="text-white text-sm text-right">{formatDate(campaign.endDate)}</span>
                </div>
              )}
            </div>
