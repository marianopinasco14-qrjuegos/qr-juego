...
export async function POST(req: NextRequest) {
  try {
    ...
    // Buscar primero en Winner (juegos normales)
    ...
    // Buscar en RaffleWinner (sorteos)
    const raffleWinner = await prisma.raffleWinner.findUnique({
      where: { redemptionCode },
      include: { rafflePrize: { include: { campaign: { include: { organization: true } } } }, lead: true },
    });
    if (!raffleWinner) return NextResponse.json({ valid: false, message: "Código no encontrado." });
    if (raffleWinner.rafflePrize.campaign.organizationId !== organizationId) return NextResponse.json({ valid: false, message: "Código no corresponde a tu organización." });
    if (raffleWinner.isAlternate) return NextResponse.json({ valid: false, message: "Este código pertenece a un suplente y no puede canjearse directamente." });
    if (raffleWinner.isRedeemed) return NextResponse.json({ valid: false, alreadyRedeemed: true, message: `Ya fue canjeado el ${raffleWinner.redeemedAt?.toLocaleDateString("es-AR")}.` });
    if (new Date() > raffleWinner.expiresAt) return NextResponse.json({ valid: false, message: "Este premio ya expiró." });

    await prisma.$transaction([
      prisma.raffleWinner.update({ where: { id: raffleWinner.id }, data: { isRedeemed: true, redeemedAt: new Date(), redeemedByPin: staffPin } }),
      prisma.staffPin.update({ where: { id: staffPinRecord.id }, data: { redeemedCount: { increment: 1 } } })
    ]);

    dispatchWebhook(organizationId, "prize.redeemed", {
      redemptionCode,
      prizeName: raffleWinner.rafflePrize.title,
      customerEmail: raffleWinner.lead.email,
      staffPin,
    });
    return NextResponse.json({ valid: true, message: "¡Premio de sorteo canjeado exitosamente!", prizeName: raffleWinner.rafflePrize.title, customerEmail: raffleWinner.lead.email });
  } catch (error) {
